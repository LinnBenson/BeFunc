...
export default class ApiProessClass {
    ...
    async send( data, load = true, options = {} ) {
        ...
        try {
            const addHeader = {};
            localStorage.getItem( 'lang' ) ? addHeader['lang'] = localStorage.getItem( 'token' ) : false;
            localStorage.getItem( 'token' ) ? addHeader['Authorization'] = `Bearer ${localStorage.getItem( 'token' )}` : false;
            // 请求类型
            if ( data.post ) {
                // POST 请求
